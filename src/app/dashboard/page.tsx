...
const DashboardPage = () => {
  ...
  const fetchTotalEarnings = useCallback(async () => {
    ...
    try {
      ...
      const earnings = await contract.getCreatorEarnings();
      // Convert to number before using toFixed
      setTotalEarnings(parseFloat(ethers.formatEther(earnings)).toFixed(2));
    } catch (error) {
 ...
 console.log(userAssets)
  ...
  }, [user?.walletAddress, fetchWalletBalance, fetchUserAssets, fetchTransactions, fetchTotalEarnings]);

  if (!user) return <p>Loading...</p>;

  return (
    <div className="max-w-4xl mx-auto p-6 bg-gray-900 text-white rounded-lg shadow-lg my-10">
      <h1 className="text-2xl md:text-3xl font-semibold">{getGreeting(user.fullName)},</h1>
      ...
      <div className="mt-6 p-4 bg-gray-800 rounded-lg">
        ...
        <p className="py-1">Balance: {walletBalance} ETH</p>
        <p className="py-1">Total Earnings: {totalEarnings} ETH</p>
